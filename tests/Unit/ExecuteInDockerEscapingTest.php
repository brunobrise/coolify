...
it('prevents command injection via container id', function () {
    $result = executeInDocker('test-container; id #', 'ls -la /app');

    expect($result)
        ->toBe("docker exec 'test-container; id #' bash -c 'ls -la /app'")
        ->not->toContain('docker exec test-container;');
});
