...
class RestartProxyJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    private function buildRestartCommands(): array
    {
        ...
        $commands = $commands->merge([
            "echo 'Stopping proxy...'",
            dockerStopContainerCommand($containerName, $stopTimeout).' 2>/dev/null || true',
        ...
        if ($this->server->isSwarmManager()) {
            $commands = $commands->merge([
                ...
                'mkdir -p '.escapeshellarg("{$proxy_path}/dynamic"),
                'cd '.escapeshellarg($proxy_path),
            ...
            $commands = $commands->merge([
                ...
                'mkdir -p '.escapeshellarg("{$proxy_path}/dynamic"),
                'cd '.escapeshellarg($proxy_path),
                'printf %s '.escapeshellarg($caddyfile).' > '.escapeshellarg("{$proxy_path}/dynamic/Caddyfile"),
                "echo 'Creating required Docker Compose file.'",
